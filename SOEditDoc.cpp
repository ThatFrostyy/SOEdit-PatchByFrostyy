...
void CSOEditDoc::OnClearAll(bool question)
{
	if(m_Model && m_Model -> m_skeleton && m_Model -> m_skeleton -> m_bonelist && question)
	{
		...
		#else
			if(MessageBox(AfxGetApp() -> m_pMainWnd -> m_hWnd, "Вы подтверждаете очистку сцены?", "QUESTION: CSOEditDoc::OnClearAll", MB_YESNO | MB_ICONQUESTION) != IDYES)
		#endif
		{return;}
	}
	if(m_Model)
	...
	#else
		hTreeItem = pTreeCtrl -> InsertItem("Скелет", TVI_ROOT, NULL);
	#endif
	...
	#else
		hTreeItem = pTreeCtrl -> InsertItem("Волюмы", TVI_ROOT, NULL);
	#endif
	pTreeCtrl -> SetCheck(hTreeItem, TRUE);
	...
	#else
		hTreeItem = pTreeCtrl -> InsertItem("Препятствия", TVI_ROOT, NULL);
	#endif
	pTreeCtrl -> SetCheck(hTreeItem, TRUE);
	...
	#else
		hTreeItem = pTreeCtrl -> InsertItem("Области", TVI_ROOT, NULL);
	#endif
	pTreeCtrl -> SetCheck(hTreeItem, TRUE);
			...
			while(NULL != pMesh)
			{
				for(int i = 0; i < pMesh -> m_texcount; i++)
				{
					...
					else
					{
						...
						#else
							sprintf(msgText, "Кость: %s\nМэш не имеет материала.\nPLY: %s", basis -> m_Name, basis -> m_VolumeViewName);
						#endif
...
void CSOEditDoc::ProcessSkin(CBone *basis)
{
	...
	if(!basis -> m_VolumeView)
	{
		...
		if (skin)
		{
			if(skin -> m_VolumeView && skin -> m_VolumeView -> SKINNED && false)//отключил сепарацию мэшей
			{
...
void CSOEditDoc::OnFileOpen()
{
	...
	#else
		static char szFilter[] = "MDL-Файл (*.mdl)|*.mdl|";
		static char szTitle[] = "Открытие файла модели";
	#endif
	bool p = false;
...
void CSOEditDoc::OnAnmLoad()
{
	...
	if(!strlen(pFrameWnd -> m_CurMdlPath))
	{
		...
		#else
			MessageBox(AfxGetApp() -> m_pMainWnd -> m_hWnd, "Модель не загружена!\nЕсли модель делается с нуля, то сохрани и загрузи её.", "ERROR: CSOEditDoc::OnAnmLoad", MB_ICONHAND);
		#endif
		m_Animate = false;
		return;
	}
		...
		static char szTitle[] = "Открытие файла анимации";
	#endif
	bool p = false;
	char CurFolder[_MAX_PATH] = {0};
	if(strlen(pFrameWnd -> m_CurMdlPath))
...
void CSOEditDoc::OnVolAdd() 
{
	...
	#else
		if(IDYES == (yesno = MessageBox(AfxGetApp() -> m_pMainWnd -> m_hWnd, "Добавить волюм из файла (VOL)?", "QUESTION: CSOEditDoc::OnVolAdd", MB_YESNO | MB_ICONQUESTION)))
	#endif
	{
		#ifdef ALTERNATIVE_LANG
...
void CSOEditDoc::OnBoneAdd()
{
	if(!m_Model)
	{
		...
		#else
			MessageBox(AfxGetApp() -> m_pMainWnd -> m_hWnd, "Модель не загружена.", "ERROR: CSOEditDoc::OnBoneAdd", MB_ICONHAND);
		#endif
		m_Animate = false;
		return;
	}
	...
	#else
		if((yesno = MessageBox(AfxGetApp() -> m_pMainWnd -> m_hWnd, "Добавить мэш из файла (PLY)?", "QUESTION: CSOEditDoc::OnBoneAdd", MB_YESNO | MB_ICONQUESTION)) == IDYES)
	#endif
	{
		#ifdef ALTERNATIVE_LANG
		...
		#else
			static char szFilter[] = "PLY-файл (*.ply)|*.ply||";
			static char szTitle[] = "Открытие мэш-файла";
		#endif
		bool p = false;
		char CurFolder[_MAX_PATH] = {0};
		if(strlen(pFrameWnd -> m_CurMdlPath))
...
void CSOEditDoc::OnBoneDelete()
{
	...
	if(pBone)
	{
		...
		#else
			if(CuteMode || (MessageBox(AfxGetApp() -> m_pMainWnd -> m_hWnd, "Точно удалить эту кость?", "QUESTION: CSOEditDoc::OnBoneDelete", MB_YESNO | MB_ICONQUESTION) == IDYES))
		#endif
		{
...
void CSOEditDoc::AnalysisOfBoneBranchesByVolume(CBone *basis, CString &tmpStr, bool &binding, bool next)
{
	...
	while(pVol)
	{
		if(pVol -> boneptr == basis)
		{
			...
			#else
				tmpStr += "Волюм \"" + CString(pVol -> m_Name) + "\" привязан к кости \"" + CString(basis -> m_Name) + "\"\r\n";
			#endif
			binding = true;
			//break;
		}
...
void CSOEditDoc::OnBoneProp()
{
	...
	else
	{
		...
		#else
			MessageBox(AfxGetApp() -> m_pMainWnd -> m_hWnd, CString("Кость: " + CString(szBoneName) + " не найдена.\nПамять программы повреждена!\nПерезапусти!"), "ERROR: CSOEditDoc::OnBoneProp", MB_ICONHAND);
		#endif
	}
	pTreeCtrl -> SetFocus();
}
...
void CSOEditDoc::OnAnmEdit()
{
	if(!m_AnimBone)
	{
		...
		#else
			if(MessageBox(AfxGetApp() -> m_pMainWnd -> m_hWnd, "Анимация не загружена.\nЗагрузить?", "QUESTION: CSOEditDoc::OnAnmEdit", MB_YESNO | MB_ICONQUESTION) == IDYES)
		#endif
				{OnAnmLoad();}
	}
	if(m_AnimBone)
	{
		...
		AnimBoxUPD();
		//------------------------
		//если задания нет, то и переменная обнуляется
		//------------------------
	}
}
...
void CSOEditDoc::OnModelSave(bool mod)
{
	if(m_Model)
	{
		...
		#else
			static char szFilter[] = "MDL-Файл (*.mdl)|*.mdl|";
			static char szTitle[] = "Сохранить файл модели как:";
		#endif
		FILE *fp;
		bool p = false;
		char CurFolder[_MAX_PATH] = {0};
		...
		if(mod ? dlg.DoModal() == IDOK : true)
		{
			...
			if((fp = fopen(szFileName, "r")) != NULL)
			{
				...
				if(mod)
				{
					...
					#else
						save = MessageBox(AfxGetApp() -> m_pMainWnd -> m_hWnd, CString("Файл:\n" + CString(szFileName) + "\nуже существует. Заменить?"), "WARNING: CSOEditDoc::OnModelSave", MB_YESNO | MB_ICONEXCLAMATION);
					#endif
				}
				else
				{
					...
					#else
						save = MessageBox(AfxGetApp() -> m_pMainWnd -> m_hWnd, CString("Вы подтверждаете перезапись файла?"), "QUESTION: CSOEditDoc::OnModelSave", MB_YESNO | MB_ICONQUESTION);
					#endif
				}
				if(save != IDYES)
					{saveit = false;}
			...
			if(saveit)
			{
				...
				else
				{
					...
					#else
						MessageBox(AfxGetApp() -> m_pMainWnd -> m_hWnd, "Ошибка. Твой никнейм не указан!\r\nЗайди в свойства программы, запиши свой ник и пересохрани файл.", "ERROR: CSOEditDoc::OnFileSaveAs", MB_ICONHAND);
					#endif
...
void CSOEditDoc::OnVolDelete()
{
	...
	if(pVolume)
	{
		...
		#else
			if(MessageBox(AfxGetApp() -> m_pMainWnd -> m_hWnd, "Точно удалить этот волюм?", "QUESTION: CSOEditDoc::OnVolDelete", MB_YESNO | MB_ICONQUESTION) == IDYES)
		#endif
			{
				if(pVolume -> next)
...
void CSOEditDoc::NameCountAndLinkControl(void)
{
	...
	for(;NameList[i]; i++)
	{
		...
		if(cou > 0)
		{
			...
			#else
				tmp_str.Format("\n%d повтор (а, ов) имени кости \"%s\"", cou, NameList[i]);
			#endif
			mesg += tmp_str;
		}
	}
	...
	if(m_Model -> m_VolumeList)
	{
		...
		for(i = 0; NameList[i]; i++)
		{
			...
			if(cou > 0)
			{
				...
				#else
					tmp_str.Format("\n%d повтор (а, ов) имени волюма \"%s\"", cou, NameList[i]);
				#endif
				mesg += tmp_str;
			}
		}
		...
		while(pVol)
		{
			if(pVol -> m_Bone)
			{
				...
				if(!pFB)
				{
					...
					#else
						tmp_str.Format("\nВолюм \"%s\" привязан к несуществующей кости \"%s\"", pVol -> m_Name, pVol -> m_Bone);
					#endif
					mesg += tmp_str;
			...
			else
			{
				...
				#else
					tmp_str.Format("\nВолюм \"%s\" не привязан к кости.", pVol -> m_Name);
				#endif
				mesg += tmp_str;
				fatal_vol_errors = true;
			}
...
void CSOEditDoc::VolumesConnections(void) //Это костыль. Не смог починить конфиг библиотек для CBone.h
{
...
//Всё должно работать не "хорошо", а так как в игре. Линковка в игре - не очевидна...
...
void CSOEditDoc::On_Bone_Copy()//если была выделена саб-кость, то нужно копировать парент
{
	Copy_Mode = 1;
...
void CSOEditDoc::ClipboardToCode_block()
{
	...
	if(!pBone)
		{return;}
	Code_block = _T("");
	if(OpenClipboard(AfxGetApp() -> m_pMainWnd -> m_hWnd))//открываем буфер обмена
	{
...
void CSOEditDoc::Code_blockToClipboard()
{
	...
	if(OpenClipboard(AfxGetApp() -> m_pMainWnd -> m_hWnd))//открываем буфер обмена
	{
	   ...
	   hgBuffer = GlobalAlloc(GMEM_DDESHARE, Code_block.GetLength() + 1);//выделяем память
...
void CSOEditDoc::ANM_Tool(bool mod)
{
	...
	#else
		pStatic -> SetWindowText("Режим: Редактирование скелета");
	#endif
...
void CSOEditDoc::OnModEditing()
{
	...
	if(m_Frame > 0)
	{
		if(!AnmEditingMod)
		{
			...
			#else
				pStatic -> SetWindowText("Режим: Редактирование анимации");
			#endif
		}
		AnmEditingMod = true;
	}
...
//Ах, какая невезуха-а-а. Абсолютно нету рифмы...
...
void CSOEditDoc::OnAnmSave()
{
	...
	#else
		static char szFilter[] = "ANM-Файл (*.anm)|*.anm|";
		static char szTitle[] = "Сохранить файл анимации как:";
	#endif
	CMainFrame *pFrameWnd = (CMainFrame *)AfxGetMainWnd();
	bool p = false;
	char CurFolder[_MAX_PATH] = {0};
	...
	if(mod ? dlg.DoModal() == IDOK : true)
	{
		...
		if((fp = fopen(szAnimPath, "r")) != NULL)
		{
			...
			if(mod)
			{
				...
				#else
					save = MessageBox(AfxGetApp() -> m_pMainWnd -> m_hWnd, CString("Файл:\n" + CString(szAnimPath) + "\nуже существует. Заменить?"), "WARNING: CSOEditDoc::OnAnmSave", MB_YESNO | MB_ICONEXCLAMATION);
				#endif
			}
			else
			{
				...
				#else
					save = MessageBox(AfxGetApp() -> m_pMainWnd -> m_hWnd, CString("Вы подтверждаете перезапись файла анимации?"), "QUESTION: CSOEditDoc::OnAnmSave", MB_YESNO | MB_ICONQUESTION);
				#endif
			}
			if(save != IDYES)
				{saveit = false;}
...
void CSOEditDoc::OnDelBoneFromAnm()
{
	...
	if(Finded)//если кость одна - то удалять нельзя
