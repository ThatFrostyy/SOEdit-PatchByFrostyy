...
bool CAnimBone::Process(FILE *fp)
{
	...
	if(strnicmp("EANM", tag, 4))
		{return false;} // This is not a valid ANM file
				...
				switch(tagindx)
				{
					...
					case 7: //MESH yat`...
					{
						if(true)
						{
							...
							#else
								MessageBoxA(AfxGetApp() -> m_pMainWnd -> m_hWnd, "Файл анимации содержит данные о мэше.\nЭто не поддерживается программой ;..(", "ERROR: CAnimBone::Process", MB_ICONHAND);
							#endif
					...
					case 6: //FRM2
					{
						...
						for(int j = 0; j < keyframe_chunks; j++)
						{
							...
							if(chunk_flags > 63)
							{
								...
								#else
									MessageBoxA(AfxGetApp() -> m_pMainWnd -> m_hWnd, "Неопознанный флаг!!!", "ERROR: CAnimBone::Process", MB_ICONHAND);
								#endif
								delete[] bone_vertex_size;
								delete[] m_Frames;
								m_Frames = NULL;
	...
	//ловим нулевые кадры без ориентации
	...
	for(int CurBone = 0; CurBone < m_BoneCnt; CurBone++)
	{
		for(CurFrm = 0; CurFrm < m_FrameCnt; CurFrm++)
		{
			...
			find_sub = NULL;
			Seeker = CurFrm;
			char subcopy = 0;// 0 - копировать не требуется, 1 - полное копирование (по возможности), 2 - частичное копирование.
			while(true)
			{
				if(Seeker < 0)
					{break;}
	...
	if(invert)//фмксим инверсный кватернион на первом кадре рендера
	{
...
void CAnimBone::ProcessSave(FILE *fp)
{
	...
	for(BoneIndx = 0; BoneIndx < m_BoneCnt; BoneIndx++)
	{
		...
		fwrite((int*)&namesize, sizeof(int), 1, fp);//А почему для PLY-файлов это char??
		fwrite(m_BoneMap[BoneIndx], namesize, 1, fp);
	}
	//первый кадр обязателен
	fwrite("FRMN", 4, 1, fp);
...
{
	int NBone = 0;//индекс новой кости
	if(!m_BoneMap)//если костей нет
	{
	...
	CAnimSub *t_sub = NULL;
	int n = 0;
	if(!m_FrameCnt)//если кадров нет
		{m_Frames = new CAnimFrame[1];}
	do//добавляем кость во все кадры
	{
...
int CAnimBone::OnAddClone(int indx)//индекс имени
{
	int NBone = 0;//индекс новой кости
	if(!m_BoneMap)
		{return -1;}
